<?php get_header(); ?>

<main id="single-activities">
    <div class="breadcrump font-20-700">
        <a class="home link" href="<?php echo HOME; ?>">
            <img src="<?php echo T_DIRE_URI; ?>/assets/img/home-icon.svg">
        </a>
        <img class="right-symbol" src="<?php echo T_DIRE_URI; ?>/assets/img/right-symbol.svg">
        <a class="link" href="<?php echo HOME . 'activities'; ?>">みんなの活動</a>
        <img class="right-symbol" src="<?php echo T_DIRE_URI; ?>/assets/img/right-symbol.svg">
        <p class="current-page">
            <?php the_title(); ?>
        </p>
    </div>

    <section class="activities-content">
        <div class="container">
            <div class="include-sidebar">
                <div class="left-side">
                    <h3 class="title font-32-700">
                        <?php the_title(); ?>
                    </h3>
                    <div class="favorite-wrapper">
                        <img src="<?php echo T_DIRE_URI; ?>/assets/img/favorite-symbol.svg">
                        <span class="number">12</span>
                    </div>
                    <?php
                    $company_name = get_field('company');
                    $province = get_field("province");
                    ?>
                    <div class="company-province font-16-500">
                        <?php if ($company_name)
                            echo $company_name; ?>/
                        <?php if ($province)
                            echo $province; ?>
                    </div>
                    <div class="category-socials">
                        <?php $post_cats = get_the_terms(get_the_ID(), 'activity-category'); ?>
                        <?php if ($post_cats): ?>
                            <div class="categories">
                                <?php foreach ($post_cats as $post_cat): ?>
                                    <p class="btn category active"><span>
                                            <?php echo $post_cat->name; ?>
                                        </span></p>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                        <?php if (shortcode_exists('addtoany')): ?>
                            <div class="addtoany">
                                <?php echo do_shortcode('[addtoany]'); ?>
                            </div>
                        <?php endif; ?>
                    </div>

                    <?php if (get_field('article-style') == '画像を埋め込んだインタビュー形式'): ?>
                        <div class="activity-detail by-image font-16-400">
                            <?php $head_text = get_field('header-text'); ?>
                            <?php if ($head_text): ?>
                                <p class="header-text font-20-700">
                                    <?php echo $head_text; ?>
                                </p>
                            <?php endif; ?>
                            <img src="<?php echo get_field('profile-image'); ?>" class="profile-image">
                            <?php if (get_field('profile-text')): ?>
                                <div class="profile-pannel">
                                    <h5 class="name font-18-700">
                                        <?php echo get_field('profile-name'); ?>
                                    </h5>
                                    <p class="profile-text font-16-400">
                                        <?php echo get_field('profile-text'); ?>
                                    </p>
                                </div>
                            <?php endif; ?>
                            <div class="content-wrapper font-16-400">
                                <?php the_content(); ?>
                            </div>
                        </div>
                    <?php endif; ?>

                    <?php if (get_field('article-style') == '動画セミナー一覧を埋め込んだパターン'): ?>
                        <div class="activity-detail by-video font-16-400">
                            <div class="content-wrapper font-16-400">
                                <?php the_content(); ?>
                            </div>
                            <h2>動画セミナー 一覧</h2>
                            <br>
                            <?php if (have_rows('video-list')): ?>
                                <ul class="video-list">
                                    <?php while (have_rows('video-list')):
                                        the_row(); ?>
                                        <li class="item">
                                            <iframe src="<?php echo get_sub_field('video-url'); ?>"></iframe>
                                            <h4 class="video-title font-16-700">
                                                <?php echo get_sub_field('video-title'); ?>
                                                <h4>
                                        </li>
                                    <?php endwhile; ?>
                                </ul>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>

                    <?php if (get_field('article-style') == '勉強会（セミナー）の開催告知パターン'): ?>
                        <div class="activity-detail seminar font-16-400">

                            <?php $head_text = get_field('header-text'); ?>
                            <?php if ($head_text): ?>
                                <p class="head-desc font-16-400">
                                    <?php echo $head_text; ?>
                                </p>
                            <?php endif; ?>

                            <?php
                            $seminar_overview = get_field('seminar-overview');
                            $starting_time = $seminar_overview['starting-time'];
                            $venue = $seminar_overview['venue'];
                            $participation_fee = $seminar_overview['fee'];
                            $target_audience = $seminar_overview['target-audience'];
                            $detail = $seminar_overview['detail'];
                            ?>
                            <?php if ($seminar_overview): ?>
                                <h2>セミナー概要</h2>
                                <br>
                                <ul class="overview-table">
                                    <li class="record">
                                        <p class="table-title font-16-700">
                                            開催日時
                                        </p>
                                        <p class="content font-16-400">
                                            <?php if ($starting_time): ?>
                                                <?php echo $starting_time; ?>
                                            <?php endif; ?>
                                        </p>
                                    </li>
                                    <li class="record">
                                        <p class="table-title font-16-700">
                                            開催場所
                                        </p>
                                        <p class="content font-16-400">
                                            <?php if ($venue): ?>
                                                <?php echo $venue; ?>
                                            <?php endif; ?>
                                        </p>
                                    </li>
                                    <li class="record">
                                        <p class="table-title font-16-700">
                                            参加費用
                                        </p>
                                        <p class="content font-16-400">
                                            <?php if ($participation_fee): ?>
                                                <?php echo $participation_fee; ?>
                                            <?php endif; ?>
                                        </p>
                                    </li>
                                    <li class="record">
                                        <p class="table-title font-16-700">
                                            対象者
                                        </p>
                                        <p class="content font-16-400">
                                            <?php if ($target_audience): ?>
                                                <?php echo $target_audience; ?>
                                            <?php endif; ?>
                                        </p>
                                    </li>
                                    <li class="record">
                                        <p class="table-title font-16-700">
                                            詳細
                                        </p>
                                        <p class="content font-16-400">
                                            <?php if ($detail): ?>
                                                <?php echo $detail; ?>
                                            <?php endif; ?>
                                        </p>
                                    </li>
                                </ul>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>

                    <?php if (get_field('article-style') == '（会員登録したら見れる）パターン'): ?>
                        <div class="activity-detail by-member font-16-400">
                            <?php $head_text = get_field('header-text'); ?>
                            <?php if ($head_text): ?>
                                <p class="head-text font-16-400">
                                    <?php echo $head_text; ?>
                                </p>
                                <div class="security-label font-16-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        fill="none">
                                        <path
                                            d="M19.5 7.5H16.5V5.25C16.5 4.05653 16.0259 2.91193 15.182 2.06802C14.3381 1.22411 13.1935 0.75 12 0.75C10.8065 0.75 9.66193 1.22411 8.81802 2.06802C7.97411 2.91193 7.5 4.05653 7.5 5.25V7.5H4.5C4.10218 7.5 3.72064 7.65804 3.43934 7.93934C3.15804 8.22064 3 8.60218 3 9V19.5C3 19.8978 3.15804 20.2794 3.43934 20.5607C3.72064 20.842 4.10218 21 4.5 21H19.5C19.8978 21 20.2794 20.842 20.5607 20.5607C20.842 20.2794 21 19.8978 21 19.5V9C21 8.60218 20.842 8.22064 20.5607 7.93934C20.2794 7.65804 19.8978 7.5 19.5 7.5ZM12 15.375C11.7775 15.375 11.56 15.309 11.375 15.1854C11.19 15.0618 11.0458 14.8861 10.9606 14.6805C10.8755 14.475 10.8532 14.2488 10.8966 14.0305C10.94 13.8123 11.0472 13.6118 11.2045 13.4545C11.3618 13.2972 11.5623 13.19 11.7805 13.1466C11.9988 13.1032 12.225 13.1255 12.4305 13.2106C12.6361 13.2958 12.8118 13.44 12.9354 13.625C13.059 13.81 13.125 14.0275 13.125 14.25C13.125 14.5484 13.0065 14.8345 12.7955 15.0455C12.5845 15.2565 12.2984 15.375 12 15.375ZM15 7.5H9V5.25C9 4.45435 9.31607 3.69129 9.87868 3.12868C10.4413 2.56607 11.2044 2.25 12 2.25C12.7956 2.25 13.5587 2.56607 14.1213 3.12868C14.6839 3.69129 15 4.45435 15 5.25V7.5Z"
                                            fill="#52432C" />
                                    </svg>
                                    続きを読むには会員登録をしてください
                                </div>
                                <div class="number-character font-14-400">残り1,000字</div>
                                <a class="btn btn-register" href=""><span>会員登録（無料）</span></a>
                                <a class="login font-14-400" href="">会員の方はこちらからログイン</a>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>

                    <div class="about-activities">
                        <p class="label font-18-700">この活動について</p>
                        <div class="btn-wrapper">
                            <p class="btn btn-green">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none">
                                    <path
                                        d="M17.625 8.25005C17.2357 8.2495 16.8513 8.33633 16.5 8.50412V5.62505C16.4999 5.17016 16.3816 4.72309 16.1566 4.3277C15.9317 3.93231 15.6078 3.60216 15.2169 3.36962C14.8259 3.13708 14.3812 3.01013 13.9264 3.00122C13.4716 2.99232 13.0223 3.10175 12.6225 3.3188C12.43 2.72404 12.0316 2.21748 11.499 1.89027C10.9663 1.56306 10.3344 1.43669 9.71687 1.53388C9.09934 1.63108 8.53677 1.94546 8.13038 2.42047C7.72399 2.89548 7.50045 3.49992 7.5 4.12505V4.75412C7.14873 4.58633 6.76429 4.4995 6.375 4.50005C5.67881 4.50005 5.01113 4.77662 4.51884 5.2689C4.02656 5.76118 3.75 6.42886 3.75 7.12505V14.2501C3.75 16.4381 4.61919 18.5365 6.16637 20.0837C7.71354 21.6309 9.81196 22.5001 12 22.5001C14.188 22.5001 16.2865 21.6309 17.8336 20.0837C19.3808 18.5365 20.25 16.4381 20.25 14.2501V10.8751C20.25 10.1789 19.9734 9.51118 19.4812 9.0189C18.9889 8.52662 18.3212 8.25005 17.625 8.25005ZM18.75 14.2501C18.75 16.0403 18.0388 17.7572 16.773 19.023C15.5071 20.2889 13.7902 21.0001 12 21.0001C10.2098 21.0001 8.4929 20.2889 7.22703 19.023C5.96116 17.7572 5.25 16.0403 5.25 14.2501V7.12505C5.25 6.82669 5.36853 6.54054 5.5795 6.32956C5.79048 6.11858 6.07663 6.00005 6.375 6.00005C6.67337 6.00005 6.95952 6.11858 7.1705 6.32956C7.38147 6.54054 7.5 6.82669 7.5 7.12505V11.2501C7.5 11.449 7.57902 11.6397 7.71967 11.7804C7.86032 11.921 8.05109 12.0001 8.25 12.0001C8.44891 12.0001 8.63968 11.921 8.78033 11.7804C8.92098 11.6397 9 11.449 9 11.2501V4.12505C9 3.82668 9.11853 3.54054 9.3295 3.32956C9.54048 3.11858 9.82663 3.00005 10.125 3.00005C10.4234 3.00005 10.7095 3.11858 10.9205 3.32956C11.1315 3.54054 11.25 3.82668 11.25 4.12505V10.5001C11.25 10.699 11.329 10.8897 11.4697 11.0304C11.6103 11.171 11.8011 11.2501 12 11.2501C12.1989 11.2501 12.3897 11.171 12.5303 11.0304C12.671 10.8897 12.75 10.699 12.75 10.5001V5.62505C12.75 5.32669 12.8685 5.04054 13.0795 4.82956C13.2905 4.61858 13.5766 4.50005 13.875 4.50005C14.1734 4.50005 14.4595 4.61858 14.6705 4.82956C14.8815 5.04054 15 5.32669 15 5.62505V12.0629C13.9521 12.2415 13.001 12.7848 12.3148 13.5967C11.6286 14.4087 11.2515 15.437 11.25 16.5001C11.25 16.699 11.329 16.8897 11.4697 17.0304C11.6103 17.171 11.8011 17.2501 12 17.2501C12.1989 17.2501 12.3897 17.171 12.5303 17.0304C12.671 16.8897 12.75 16.699 12.75 16.5001C12.75 15.7044 13.0661 14.9413 13.6287 14.3787C14.1913 13.8161 14.9544 13.5001 15.75 13.5001C15.9489 13.5001 16.1397 13.421 16.2803 13.2804C16.421 13.1397 16.5 12.949 16.5 12.7501V10.8751C16.5 10.5767 16.6185 10.2905 16.8295 10.0796C17.0405 9.86858 17.3266 9.75005 17.625 9.75005C17.9234 9.75005 18.2095 9.86858 18.4205 10.0796C18.6315 10.2905 18.75 10.5767 18.75 10.8751V14.2501Z"
                                        fill="white" />
                                </svg>
                                <span>検索する</span>
                            </p>
                            <p class="btn btn-green">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none">
                                    <path
                                        d="M13.125 16.875C13.125 17.0975 13.059 17.315 12.9354 17.5C12.8118 17.685 12.6361 17.8292 12.4305 17.9144C12.225 17.9995 11.9988 18.0218 11.7805 17.9784C11.5623 17.935 11.3618 17.8278 11.2045 17.6705C11.0472 17.5132 10.94 17.3127 10.8966 17.0945C10.8532 16.8762 10.8755 16.65 10.9606 16.4445C11.0458 16.2389 11.19 16.0632 11.375 15.9396C11.56 15.816 11.7775 15.75 12 15.75C12.2984 15.75 12.5845 15.8685 12.7955 16.0795C13.0065 16.2905 13.125 16.5766 13.125 16.875ZM12 6.75C9.93188 6.75 8.25 8.26406 8.25 10.125V10.5C8.25 10.6989 8.32902 10.8897 8.46967 11.0303C8.61033 11.171 8.80109 11.25 9 11.25C9.19892 11.25 9.38968 11.171 9.53033 11.0303C9.67099 10.8897 9.75 10.6989 9.75 10.5V10.125C9.75 9.09375 10.7597 8.25 12 8.25C13.2403 8.25 14.25 9.09375 14.25 10.125C14.25 11.1562 13.2403 12 12 12C11.8011 12 11.6103 12.079 11.4697 12.2197C11.329 12.3603 11.25 12.5511 11.25 12.75V13.5C11.25 13.6989 11.329 13.8897 11.4697 14.0303C11.6103 14.171 11.8011 14.25 12 14.25C12.1989 14.25 12.3897 14.171 12.5303 14.0303C12.671 13.8897 12.75 13.6989 12.75 13.5V13.4325C14.46 13.1184 15.75 11.7544 15.75 10.125C15.75 8.26406 14.0681 6.75 12 6.75ZM21.75 12C21.75 13.9284 21.1782 15.8134 20.1068 17.4168C19.0355 19.0202 17.5127 20.2699 15.7312 21.0078C13.9496 21.7458 11.9892 21.9389 10.0979 21.5627C8.20656 21.1865 6.46928 20.2579 5.10571 18.8943C3.74215 17.5307 2.81355 15.7934 2.43735 13.9021C2.06114 12.0108 2.25422 10.0504 2.99218 8.26884C3.73013 6.48726 4.97982 4.96451 6.58319 3.89317C8.18657 2.82183 10.0716 2.25 12 2.25C14.585 2.25273 17.0634 3.28084 18.8913 5.10872C20.7192 6.93661 21.7473 9.41498 21.75 12ZM20.25 12C20.25 10.3683 19.7661 8.77325 18.8596 7.41655C17.9531 6.05984 16.6646 5.00242 15.1571 4.37799C13.6497 3.75357 11.9909 3.59019 10.3905 3.90852C8.79017 4.22685 7.32016 5.01259 6.16637 6.16637C5.01259 7.32015 4.22685 8.79016 3.90853 10.3905C3.5902 11.9908 3.75358 13.6496 4.378 15.1571C5.00242 16.6646 6.05984 17.9531 7.41655 18.8596C8.77326 19.7661 10.3683 20.25 12 20.25C14.1873 20.2475 16.2843 19.3775 17.8309 17.8309C19.3775 16.2843 20.2475 14.1873 20.25 12Z"
                                        fill="white" />
                                </svg>
                                <span>質問したい</span>
                            </p>
                            <p class="btn btn-green">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none">
                                    <path
                                        d="M20.25 9.75C20.25 9.94891 20.171 10.1397 20.0303 10.2803C19.8897 10.421 19.6989 10.5 19.5 10.5C19.3011 10.5 19.1103 10.421 18.9697 10.2803C18.829 10.1397 18.75 9.94891 18.75 9.75C18.75 7.95979 18.0388 6.2429 16.773 4.97703C15.5071 3.71116 13.7902 3 12 3C10.2098 3 8.4929 3.71116 7.22703 4.97703C5.96116 6.2429 5.25 7.95979 5.25 9.75C5.25 12.2531 6.04969 13.0238 6.89719 13.8412C7.70719 14.625 8.625 15.5072 8.625 17.625C8.625 18.5201 8.98058 19.3785 9.61351 20.0115C10.2464 20.6444 11.1049 21 12 21C12.96 21 13.7297 20.61 14.4216 19.7728C14.4835 19.6947 14.5604 19.6298 14.6478 19.5818C14.7352 19.5337 14.8312 19.5036 14.9304 19.4932C15.0295 19.4827 15.1297 19.4921 15.2252 19.5209C15.3206 19.5496 15.4094 19.5971 15.4863 19.6605C15.5632 19.7239 15.6267 19.8021 15.673 19.8903C15.7194 19.9786 15.7477 20.0752 15.7563 20.1745C15.7648 20.2738 15.7535 20.3739 15.723 20.4688C15.6925 20.5636 15.6433 20.6515 15.5784 20.7272C14.6072 21.9037 13.4062 22.5 12 22.5C10.7075 22.4985 9.46842 21.9844 8.5545 21.0705C7.64058 20.1566 7.12649 18.9175 7.125 17.625C7.125 16.1447 6.5925 15.6309 5.85562 14.9213C4.91812 14.0156 3.75 12.8906 3.75 9.75C3.75 7.56196 4.61919 5.46354 6.16637 3.91637C7.71354 2.36919 9.81196 1.5 12 1.5C14.188 1.5 16.2865 2.36919 17.8336 3.91637C19.3808 5.46354 20.25 7.56196 20.25 9.75ZM16.6753 15.1012C16.59 15.0518 16.4958 15.0197 16.3981 15.0066C16.3003 14.9936 16.201 15 16.1057 15.0255C16.0105 15.0509 15.9212 15.0949 15.8429 15.1548C15.7647 15.2148 15.699 15.2896 15.6497 15.375C15.5672 15.5181 15.4397 15.6299 15.2872 15.6931C15.1346 15.7564 14.9654 15.7675 14.8059 15.7247C14.6464 15.682 14.5054 15.5878 14.4049 15.4568C14.3044 15.3257 14.2499 15.1652 14.25 15C14.25 14.1253 14.7019 13.5225 15.225 12.825C15.8222 12.0281 16.5 11.1253 16.5 9.75C16.5 8.55653 16.0259 7.41193 15.182 6.56802C14.3381 5.72411 13.1935 5.25 12 5.25C10.8065 5.25 9.66193 5.72411 8.81802 6.56802C7.97411 7.41193 7.5 8.55653 7.5 9.75C7.5 9.94891 7.57902 10.1397 7.71967 10.2803C7.86032 10.421 8.05109 10.5 8.25 10.5C8.44891 10.5 8.63968 10.421 8.78033 10.2803C8.92098 10.1397 9 9.94891 9 9.75C9 8.95435 9.31607 8.19129 9.87868 7.62868C10.4413 7.06607 11.2044 6.75 12 6.75C12.7956 6.75 13.5587 7.06607 14.1213 7.62868C14.6839 8.19129 15 8.95435 15 9.75C15 10.6247 14.5481 11.2275 14.025 11.925C13.4278 12.7219 12.75 13.6247 12.75 15C12.7502 15.4952 12.9137 15.9764 13.2152 16.3692C13.5167 16.762 13.9393 17.0444 14.4176 17.1726C14.8959 17.3007 15.4031 17.2676 15.8606 17.0782C16.3181 16.8888 16.7004 16.5537 16.9481 16.125C17.0475 15.953 17.0746 15.7486 17.0234 15.5567C16.9723 15.3648 16.8471 15.201 16.6753 15.1012Z"
                                        fill="white" />
                                </svg>
                                <span>話を聞いてみたい</span>
                            </p>
                            <p class="btn btn-green">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none">
                                    <path
                                        d="M22.95 14.1002C22.8712 14.1593 22.7816 14.2023 22.6862 14.2267C22.5908 14.2511 22.4915 14.2566 22.394 14.2426C22.2965 14.2287 22.2027 14.1957 22.1179 14.1455C22.0332 14.0953 21.9591 14.029 21.9 13.9502C21.448 13.3425 20.8596 12.8495 20.1822 12.5108C19.5048 12.1721 18.7574 11.9972 18 12.0002C17.8526 12.0002 17.7083 11.9567 17.5855 11.8751C17.4626 11.7936 17.3664 11.6776 17.3091 11.5417C17.2702 11.4495 17.2501 11.3503 17.2501 11.2502C17.2501 11.15 17.2702 11.0509 17.3091 10.9586C17.3664 10.8227 17.4626 10.7068 17.5855 10.6252C17.7083 10.5437 17.8526 10.5002 18 10.5002C18.4209 10.5001 18.8332 10.3821 19.1903 10.1594C19.5474 9.93677 19.8349 9.61844 20.0201 9.24058C20.2054 8.86273 20.2809 8.4405 20.2382 8.02185C20.1955 7.6032 20.0363 7.20493 19.7786 6.87225C19.5209 6.53958 19.175 6.28584 18.7803 6.13987C18.3856 5.9939 17.9579 5.96154 17.5458 6.04646C17.1336 6.13139 16.7535 6.3302 16.4487 6.62031C16.1439 6.91043 15.9265 7.28021 15.8213 7.68767C15.7967 7.78308 15.7535 7.87271 15.6942 7.95144C15.635 8.03017 15.5608 8.09645 15.4759 8.14651C15.391 8.19656 15.2971 8.22941 15.1995 8.24318C15.102 8.25694 15.0026 8.25135 14.9072 8.22673C14.8118 8.20211 14.7222 8.15893 14.6435 8.09967C14.5647 8.04041 14.4984 7.96622 14.4484 7.88134C14.3983 7.79646 14.3655 7.70256 14.3517 7.60498C14.338 7.50741 14.3435 7.40808 14.3682 7.31267C14.5142 6.74765 14.7901 6.22451 15.1738 5.78487C15.5576 5.34523 16.0387 5.00124 16.5788 4.78025C17.119 4.55926 17.7032 4.46738 18.2851 4.51191C18.867 4.55645 19.4304 4.73618 19.9306 5.0368C20.4308 5.33742 20.8539 5.75063 21.1663 6.24356C21.4787 6.73648 21.6718 7.29552 21.7301 7.87618C21.7884 8.45684 21.7104 9.0431 21.5023 9.58831C21.2942 10.1335 20.9617 10.6226 20.5313 11.0167C21.5511 11.4583 22.4377 12.159 23.1028 13.0492C23.1619 13.1282 23.2049 13.2181 23.2292 13.3137C23.2535 13.4093 23.2587 13.5088 23.2445 13.6064C23.2303 13.704 23.197 13.7979 23.1465 13.8826C23.0959 13.9673 23.0292 14.0413 22.95 14.1002ZM17.8988 19.8752C17.9531 19.9605 17.9895 20.056 18.0059 20.1558C18.0223 20.2556 18.0184 20.3577 17.9943 20.456C17.9703 20.5542 17.9266 20.6466 17.8659 20.7275C17.8052 20.8084 17.7288 20.8762 17.6412 20.9268C17.5536 20.9775 17.4567 21.0099 17.3563 21.022C17.2559 21.0342 17.154 21.026 17.0569 20.9978C16.9597 20.9696 16.8693 20.9221 16.791 20.8581C16.7127 20.794 16.6482 20.7148 16.6013 20.6252C16.1288 19.8252 15.456 19.1622 14.6491 18.7016C13.8422 18.241 12.9291 17.9988 12 17.9988C11.0709 17.9988 10.1579 18.241 9.35102 18.7016C8.54412 19.1622 7.87125 19.8252 7.39879 20.6252C7.3519 20.7148 7.28737 20.794 7.20908 20.8581C7.13078 20.9221 7.04034 20.9696 6.94319 20.9978C6.84605 21.026 6.7442 21.0342 6.64379 21.022C6.54338 21.0099 6.44647 20.9775 6.35889 20.9268C6.27132 20.8762 6.19489 20.8084 6.1342 20.7275C6.07352 20.6466 6.02983 20.5542 6.00576 20.456C5.9817 20.3577 5.97775 20.2556 5.99417 20.1558C6.01059 20.056 6.04702 19.9605 6.10129 19.8752C6.82841 18.6258 7.9371 17.6427 9.26441 17.0702C8.51753 16.4983 7.96863 15.7068 7.69485 14.8069C7.42108 13.9069 7.4362 12.9438 7.73808 12.053C8.03997 11.1621 8.61346 10.3882 9.37792 9.84005C10.1424 9.29194 11.0594 8.99716 12 8.99716C12.9407 8.99716 13.8577 9.29194 14.6222 9.84005C15.3866 10.3882 15.9601 11.1621 16.262 12.053C16.5639 12.9438 16.579 13.9069 16.3052 14.8069C16.0315 15.7068 15.4825 16.4983 14.7357 17.0702C16.063 17.6427 17.1717 18.6258 17.8988 19.8752ZM12 16.5002C12.5934 16.5002 13.1734 16.3242 13.6667 15.9946C14.1601 15.6649 14.5446 15.1964 14.7717 14.6482C14.9987 14.1 15.0582 13.4968 14.9424 12.9149C14.8266 12.333 14.5409 11.7984 14.1214 11.3788C13.7018 10.9593 13.1673 10.6736 12.5853 10.5578C12.0034 10.4421 11.4002 10.5015 10.852 10.7285C10.3038 10.9556 9.83528 11.3401 9.50563 11.8335C9.17599 12.3268 9.00004 12.9068 9.00004 13.5002C9.00004 14.2958 9.31611 15.0589 9.87872 15.6215C10.4413 16.1841 11.2044 16.5002 12 16.5002ZM6.75004 11.2502C6.75004 11.0513 6.67102 10.8605 6.53037 10.7198C6.38972 10.5792 6.19895 10.5002 6.00004 10.5002C5.57922 10.5001 5.16685 10.3821 4.80976 10.1594C4.45267 9.93677 4.16518 9.61844 3.97994 9.24058C3.79471 8.86273 3.71915 8.4405 3.76185 8.02185C3.80455 7.6032 3.9638 7.20493 4.22152 6.87225C4.47923 6.53958 4.82507 6.28584 5.21976 6.13987C5.61445 5.9939 6.04216 5.96154 6.45432 6.04646C6.86648 6.13139 7.24656 6.3302 7.55139 6.62031C7.85622 6.91043 8.07359 7.28021 8.17879 7.68767C8.22852 7.88036 8.35276 8.04541 8.52418 8.14651C8.6956 8.2476 8.90016 8.27646 9.09285 8.22673C9.28555 8.177 9.4506 8.05276 9.55169 7.88134C9.65279 7.70992 9.68164 7.50536 9.63192 7.31267C9.48589 6.74765 9.21002 6.22451 8.82624 5.78487C8.44246 5.34523 7.96137 5.00124 7.42125 4.78025C6.88112 4.55926 6.29688 4.46738 5.715 4.51191C5.13312 4.55645 4.56967 4.73618 4.06947 5.0368C3.56928 5.33742 3.14614 5.75063 2.83375 6.24356C2.52135 6.73648 2.32831 7.29552 2.26998 7.87618C2.21165 8.45684 2.28965 9.0431 2.49777 9.58831C2.70589 10.1335 3.03838 10.6226 3.46879 11.0167C2.44999 11.4587 1.56446 12.1594 0.90004 13.0492C0.780569 13.2084 0.729204 13.4084 0.757247 13.6054C0.785289 13.8024 0.890442 13.9802 1.04957 14.0997C1.2087 14.2192 1.40877 14.2705 1.60577 14.2425C1.80278 14.2144 1.98057 14.1093 2.10004 13.9502C2.55209 13.3425 3.14049 12.8495 3.81788 12.5108C4.49526 12.1721 5.24271 11.9972 6.00004 12.0002C6.19895 12.0002 6.38972 11.9211 6.53037 11.7805C6.67102 11.6398 6.75004 11.4491 6.75004 11.2502Z"
                                        fill="white" />
                                </svg>
                                <span>参加したい</span>
                            </p>
                        </div>
                    </div>

                    <div class="favorite-wrapper">
                        <img src="<?php echo T_DIRE_URI; ?>/assets/img/favorite-symbol.svg">
                        <span class="number">12</span>
                    </div>

                    <?php if (shortcode_exists('addtoany')): ?>
                        <div class="addtoany">
                            <?php echo do_shortcode('[addtoany]'); ?>
                        </div>
                    <?php endif; ?>

                    <a class="btn-leftarrow" href="<?php echo HOME . 'activities'; ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                            <path
                                d="M4.46938 8.5306L9.46938 13.5306C9.61027 13.6715 9.80137 13.7506 10.0006 13.7506C10.1999 13.7506 10.391 13.6715 10.5319 13.5306C10.6728 13.3897 10.7519 13.1986 10.7519 12.9993C10.7519 12.8001 10.6728 12.609 10.5319 12.4681L6.0625 7.99997L10.5306 3.5306C10.6004 3.46083 10.6557 3.37801 10.6935 3.28686C10.7312 3.19571 10.7507 3.09801 10.7507 2.99935C10.7507 2.90069 10.7312 2.80299 10.6935 2.71184C10.6557 2.62069 10.6004 2.53786 10.5306 2.4681C10.4609 2.39833 10.378 2.34299 10.2869 2.30524C10.1957 2.26748 10.098 2.24805 9.99938 2.24805C9.90071 2.24805 9.80302 2.26748 9.71187 2.30524C9.62071 2.34299 9.53789 2.39833 9.46813 2.4681L4.46813 7.4681C4.39829 7.53786 4.34291 7.62072 4.30516 7.71193C4.26741 7.80313 4.24804 7.9009 4.24816 7.99961C4.24828 8.09832 4.26788 8.19604 4.30584 8.28715C4.3438 8.37827 4.39937 8.461 4.46938 8.5306Z"
                                fill="#104A00" />
                        </svg>
                        みんなの活動一覧
                    </a>

                    <?php $args = [
                        'post_type' => 'activity',
                        'post_status' => 'publish',
                        'paged' => $paged,
                        'posts_per_page' => 4,
                        'orderby' => 'post_date',
                        'order' => 'DESC',
                        'post__not_in' => array(get_the_ID()),
                    ];

                    $tax_query = [];

                    $tax_query[] = [
                        'taxonomy' => 'activity-category',
                        'field' => 'slug',
                        'terms' => 'recommended',
                    ];

                    if (!empty($tax_query)) {
                        $args['tax_query'] = $tax_query;
                    }

                    $custom_query = new WP_Query($args);
                    ?>
                    <?php if ($custom_query->have_posts()): ?>
                        <div class="recommended-wrapper">
                            <p class="label font-16-700">おすすめの活動</p>
                            <ul class="activities-list">
                                <?php while ($custom_query->have_posts()):
                                    $custom_query->the_post(); ?>
                                    <li class="activity-item">
                                        <a href="<?php the_permalink(); ?>">
                                            <h3 class="title font-18-700">
                                                <?php the_title(); ?>
                                            </h3>
                                            <p class="content font-15-400">
                                                <?php the_excerpt(); ?>
                                            </p>
                                            <p class="company-province font-15-700">
                                                <?php if (get_field("company"))
                                                    echo get_field('company') . '/'; ?><span>
                                                    <?php echo get_field('province'); ?>
                                                </span>
                                            </p>
                                            <?php $post_cats = get_the_terms(get_the_ID(), 'activity-category'); ?>
                                            <?php if ($post_cats): ?>
                                                <div class="categories">
                                                    <?php foreach ($post_cats as $post_cat): ?>
                                                        <p class="btn category active"><span>
                                                                <?php echo $post_cat->name; ?>
                                                            </span></p>
                                                    <?php endforeach; ?>
                                                </div>
                                            <?php endif; ?>
                                        </a>
                                    </li>
                                <?php endwhile; ?>
                            </ul>
                        </div>
                    <?php endif; ?>
                </div>
                <div class="right-sidebar">
                    <ul class="side-list">
                        <li class="side-item font-24-700">
                            補助金・助成金<br>
                            支援します
                        </li>
                        <li class="side-item font-24-700">
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </section>

</main>

<?php get_footer(); ?>